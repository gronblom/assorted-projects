...
allEqual' first [] = True 
allEqual' first (x:xs) = first == x && allEqual' first xs
...
distinct [] = True
distinct (x:xs) = foldr (\y ys -> ys && x /= y) True xs && distinct xs
...
rangeOf xs = sortedXs !! lastEl - sortedXs !! 0
...
longest' (max:maxs) [] = smallestHead max maxs
longest' maxs (x:xs)
  | xLen < maxLen =  longest' maxs xs
  | xLen == maxLen = longest' (x:maxs) xs 
  | otherwise = longest' [x] xs
  where maxLen = length $ head maxs
        xLen = length x

smallestHead min [] = min
smallestHead min (x:xs)
  | head min <= head x = smallestHead min xs
  | otherwise = smallestHead x xs
...
freqs xs = foldr (Map.alter fAlter) Map.empty xs

fAlter Nothing = (Just 1)
fAlter (Just a) = (Just (a + 1))
...
transfer from to amount bank
  ...
  | amount <= 0 = bank
  | balanceTooLow amount (Map.lookup from bank) = bank

transfer from to amount bank = alterAcc to (+) amount $ alterAcc from (-) amount bank

alterAcc :: String -> (Int -> Int -> Int) -> Int -> Map.Map String Int -> Map.Map String Int
alterAcc account op amount bank =
  case Map.lookup account bank of
    Nothing  -> bank                                   
    Just sum -> Map.insert account (op sum amount) bank

balanceTooLow amount (Just sum) = amount > sum
...
--maxIndex' ((k, v):kvs) = foldr (\(k,v) max -> if k > max then k else max) kvs v
maxIndex' xs = fst $ foldr (\(i, v) (maxI, maxV) -> if v > maxV then (i, v) else (maxI, maxV)) (head xs) xs
