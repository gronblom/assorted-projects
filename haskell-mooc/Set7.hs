...
data State = Start | Error | Finished | AddFlourOrSugar | AddEggs' | AddFlour' | AddSugar' | Mix' | Bake'
  deriving (Eq,Show)
...
step state event
  ...
  | state == Start && event == AddEggs = AddFlourOrSugar
  | state == AddFlourOrSugar && event == AddFlour = AddSugar'
  | state == AddFlourOrSugar && event == AddSugar = AddFlour'
  | state == AddSugar' && event == AddSugar = Mix'
  | state == AddFlour' && event == AddFlour = Mix'
  | state == Mix' && event == Mix = Bake'
  | state == Bake' && event == Bake = Finished
  | otherwise = Error

--step state (event:es) = Error
...
average xs = sum / fromIntegral numValues
  where sum = (foldr (+) 0.0 xs)
        numValues = length xs
...
instance (Ord a) => Semigroup (Set a) where
  Set a <> Set b = foldr (\x xs -> add x xs) (Set a) b
...
passwordAllowed pw req = case req of
                           (MinimumLength n) -> length pw >= n
                           (ContainsSome chars) -> containsSome pw chars
                           (DoesNotContain chars) -> doesNotContain pw chars
                           (And req1 req2) -> passwordAllowed pw req1 && passwordAllowed pw req2
                           (Or req1 req2) -> passwordAllowed pw req1 || passwordAllowed pw req2

containsSome pw chars = foldr (\x acc -> elem x pw || acc) False chars

doesNotContain pw chars =  not $ containsSome pw chars
...
--     ==> "(3*(1+1))"
--
-- Arithmetic String Arithmetic Arithmetic |
data Arithmetic = Lit Integer |  Sum' Arithmetic Arithmetic | Product' Arithmetic Arithmetic
  deriving Show
...
render (Product' a b) = "(" ++ render a ++ "*" ++ render b ++ ")"
render (Lit n) = show n
