...
combine f (Picture p1) (Picture p2) = Picture f'
  where f' (Coord x y) = f (p1 (Coord x y)) (p2 (Coord x y))
------------------------------------------------------------------------------
...
cut (Shape f1) (Shape f2) = Shape f
  where f (Coord x y) = f1 (Coord x y) && not (f2 (Coord x y))
...
paintSolid color (Shape s) (Picture p) = Picture f
  where f (Coord x y) = if s (Coord x y) then color else p (Coord x y)
------------------------------------------------------------------------------
...
paint (Picture p) (Shape s) (Picture f) = Picture f'
  where f' coords = if s coords then p coords else f coords
------------------------------------------------------------------------------
...
instance Transform Zoom where
  apply (Zoom n) (Picture f) = Picture f'
    where f' coords = f $ zoomCoord n coords

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
  apply FlipX (Picture f) = Picture f'
    where f' (Coord x y) = f (Coord (-1*x) y)
  apply FlipY (Picture f) = Picture f'
    where f' (Coord x y) = f (Coord x (-1*y))
...
instance (Transform a, Transform b) => Transform (Chain a b) where
  apply (Chain tr1 tr2) = (apply tr1) . (apply tr2)
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture (blur f)
  --  where f' (Coord x y) = as
          --asdf = foldr (\(Color r g b) (r' g' b') -> (r+r', g+g', b+b')) (0, 0, 0) [f (Coord x y), f (Coord (x+1) y), f (Coord (x-1) y), f (Coord x (y+1)), f (Coord x (y-1))]

blur f (Coord x y) = Color (rSum `div` 5) (gSum `div` 5) (bSum `div` 5) where
  (rSum, gSum, bSum) = foldr (\(Color r g b) (r', g', b') -> (r+r', g+g', b+b')) (0, 0, 0) [f (Coord x y), f (Coord (x+1) y), f (Coord (x-1) y), f (Coord x (y+1)), f (Coord x (y-1))]
...
instance Transform BlurMany where
  apply (BlurMany n) pic = go n
    where go 0 = pic
          go 1 = apply Blur pic
          go n = apply Blur (go (n-1))
