...
prettyPrint n queens = prettyPrint' n (sort queens) (1,1)
prettyPrint' :: Size -> [Coord] -> (Int, Int) -> String
prettyPrint' n queens (row, col)
  | row > n = ""
  | null queens = (lastColLineBreak' ".") ++ prettyPrint' n queens nextCoord
  | otherwise = case head queens of
                (x, y) | x == row && y == col -> (lastColLineBreak' "Q") ++ prettyPrint' n (tail queens) nextCoord 
                otherwise -> (lastColLineBreak' ".") ++ prettyPrint' n queens nextCoord
  where nextCoord = incCoord n (row,col)
        lastColLineBreak' = colLineBreak n col
-- not such a pretty line break solution. The model solution is of course nice: column out of range => line break and next row

colLineBreak n col char
  | n == col = char ++ "\n"
  | otherwise = char

incCoord :: Int -> (Int, Int) -> (Int, Int)
incCoord n (x,y)
  | y == n = (x+1, 1)
  | otherwise = (x, y+1)

--emptySquares prev curr = 

--nexCoord :: Coord -> Int -> Maybe Coord
--nextCord (x, y) n = 
...
danger coord queens = foldr (\x acc -> danger' coord x || acc) False queens
danger' (x1,y1) (x2, y2) = x1 == x2 || y1 == y2 || sameDiag (x1,y1) (x2, y2) || sameAntidiag (x1,y1) (x2, y2)
...
prettyPrint2 n queens = prettyPrint2' n (createCoords n) queens
prettyPrint2' n [] queens = ""
prettyPrint2' n ((row, col):coords) queens = char ++ prettyPrint2' n coords queens
  where char = lastColNewLine n col $ printChar (row,col) queens

printChar coord queens
  | elem coord queens = "Q"
  | danger coord queens = "#"
  | otherwise = "."

lastColNewLine n col char
  | n == col = char ++ "\n"
  | otherwise = char

createCoords n = [ (i,j) | i <- [1..n], j <- [1..n] ]
...
fixFirst n [] = Nothing
fixFirst n (x:[]) = Just (x:[])
fixFirst n (q:qs) = fixFirst' n q qs

fixFirst' n q qs
  | snd q > n = Nothing 
  | elem q qs || danger q qs = fixFirst' n (nextCol q) qs
  | otherwise = Just (q:qs)
...
backtrack [] = []
backtrack (s:[]) = []
backtrack (s1:s2:ss) = (nextCol s2):ss
...
-- author gronblom@gmail.com (alt email. nisargadattamaharaj@protonmail.com)