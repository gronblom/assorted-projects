...
mkCounter = do
    ref <- newIORef (0::Int)
    let inc = do modifyIORef ref (\x -> x+1)
    let get = do readIORef ref
    return (inc, get)
...
hSelectLines h nums = do
    lines <- hFetchLines h
    let selectedLines = [l | (l, lineNum) <- zip lines [1..], elem lineNum nums]
...
interact' f state = do
    op <- getLine
    let (continue, msg, state') = f (op, state)
    putStrLn msg
    if continue then do interact' f state' else return state'
