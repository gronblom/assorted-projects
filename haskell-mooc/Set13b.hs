...
mapM2 op xs ys = do mapM (\(x,y) -> op x y) (zip xs ys)
...
visit maze place = do currState <- get
                      if elem place currState
                        then return ()
                        else do put ([place] ++ currState)
                                case (lookup place maze) of
                                  Just paths -> mapM_ (visit maze) paths
                                  otherwise -> return ()                      
...
path maze place1 place2 = elem place2 reachableFromPlace1
                          where (_,reachableFromPlace1) = runState (visit maze place1) []
...
allSums :: [Int] -> [Int]
allSums [] = [0]
allSums xs = do x <- subsequences xs
                [sum x]
...
instance Functor SL where
  -- implement fmap
  fmap f (SL g) = SL g'
    where g' state0 = let (a,s,log) = runSL (SL g) state0
                          in (f a,s,log)
...
instance Monad SL where
  ...
  SL s >>= f = SL s'
    where s' state0 = let (val0,state1,log0) = runSL (SL s) state0
                          op2 = f val0
                          (val1,state2,log1) = runSL op2 state1
                          in (val1,state2,log0++log1)

-- author: gronblom@gmail.com
