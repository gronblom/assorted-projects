...
sums' curr acc i
  | curr > i = []
  | otherwise = (acc + curr) : sums' newCurr (acc + curr) i
  where newCurr = curr + 1
...
indexDefault xs i def
  | i < 0 = def
  | otherwise = indexDefault' xs i def 0

indexDefault' [] i def currIndex = def
indexDefault' (x:xs) i def currIndex = if i == currIndex then x
                                       else indexDefault' xs i def (currIndex + 1)
...
maybeMap f [] = []
maybeMap f (x:xs) = maybeMap' f (f x) xs
maybeMap' f (Nothing) xs = maybeMap f xs
maybeMap' f (Just x) xs = x : maybeMap f xs
