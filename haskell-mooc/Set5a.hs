...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix color1 color2 ) = zipWith (\c1 c2 -> (c1 + c2) / 2) (rgb color1) (rgb color2)
...
fromBin' _ End = 0
fromBin' i (I bs) =  2^i + fromBin' (i+1) bs
fromBin' i (O bs) = fromBin' (i+1) bs
...
toBin' 0 = End
toBin' n
  | remainder == 0 = O (toBin' quotient)
  | otherwise = I (toBin' quotient)
  where quotient = n `div` 2
        remainder = n `mod` 2
