...
split s = case elemIndex ' ' s of
  Just n -> Just $ (\(x, y) -> (x, tail y)) $ splitAt n s
  otherwise -> Nothing
...
checkNumber (for,sur)
  | containsNum for || containsNum sur = Nothing
  | otherwise = Just (for,sur)
  where containsNum = any (\c -> elem c "0123456789")
...
winner scores player1 player2 = do player1Score <- lookup player1 scores
...
selectSum xs is = mapM (safeIndex xs) is >>= (\xs -> return (sum xs))
...
count x = do curr <- get
             let newState = addOcc curr x
             put (newState)

addOcc :: Eq a => [(a,Int)] -> a -> [(a,Int)]
addOcc xs key = case lookup key xs of
  Just val -> (delete (key,val) xs) ++ [(key, val+1)]
  otherwise -> xs ++ [(key, 1)]
