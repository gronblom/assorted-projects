...
countValid serials = foldr helper 0 serials
  where helper s acc = if serialOk s then 1 + acc else acc
        serialOk s = (length s) >= 6 && s !! 2 == s !! 4 || s !! 3 == s !! 5
...
sumSuccess msrmnts = case successful of
  [] -> Left "no data"
  successful -> Right (sum (map (\(Right n) -> n) successful))
  where successful = filter filterRight msrmnts
        filterRight (Right _) = True
        filterRight (Left _) = False
...
open code (Locked lockCode)
  | code == lockCode = (Unlocked lockCode)
open code lock = lock
...
changeCode newCode (Unlocked code) = Unlocked newCode
...
permute [] xs = xs
permute prms xs = valMapper . sorter . permMapper xs $ prms where
  permMapper xs = map (\(x, y) -> ((xs !! x), y))
  sorter = sortBy (comparing (\(x,y) -> y))
  valMapper = map (\x -> fst x)
