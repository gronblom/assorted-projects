...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode node list) = LNode (f node) (fmap f list)
...
count el fldbl = foldr helper 0 fldbl
  where helper = (\x acc -> if x == el then acc + 1 else acc)
...
inBoth a b = foldr (helper b) [] a
  where helper b = (\x acc -> if elem x b then x:acc else acc)
...
instance Foldable List where
  ...
  foldr f initialValue (LNode el rest) = f el (foldr f initialValue rest)
------------------------------------------------------------------------------
...
instance Foldable TwoList where
  foldr f initVal TwoEmpty = initVal
  foldr f initVal (TwoNode a b rest) = f a $ f b $ (foldr f initVal rest)
...
instance Functor Tree where
  fmap f Leaf = Leaf
  fmap f (Node a treeA treeB) = (Node (f a) (fmap f treeA) (fmap f treeB))

sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a treeA treeB) = sumTree treeA <> a <> sumTree treeB
