...
instance Eq Name where
  Name n1 == Name n2 = (lower n1) == (lower n2)
  -- toLower (head a) : (tail a)
    where lower a = map toLower a--foldr (\x acc -> (toLower x) ++ acc) "" a 
...
instance Num RationalNumber where
  (RationalNumber a b) + (RationalNumber c d) = simplify (RationalNumber (bFactor*a+dFactor*c) (bFactor*b))
    where lcm' = lcm b d
          bFactor = lcm' `div` b
          dFactor = lcm' `div` d
...
------------------------------------------------------------------------------
-- Ex 12: cycling. Implement a type class Cycle that contains 
