...
valAtRoot (Node a left right) = Just a
...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node y l r) = 1 + treeSize l + treeSize r
...
treeMax Empty = 0
treeMax (Node y l r) = max y $ max (treeMax l) (treeMax r)
...
allValues condition (Node y l r) = condition y && allValues condition l && allValues condition r 
...
mapTree f (Node y l r) = Node (f y) (mapTree f l) (mapTree f r)
...
isOrdered (Node y l r) = allValues (<y) l && allValues (>y) r && isOrdered l && isOrdered r
...
walk [] (Node y l r) = Just y
walk (StepL:steps) (Node y l r) = walk steps l
walk (StepR:steps) (Node y l r) = walk steps r
...
set [] val (Node y l r) = (Node val l r)
set (StepL:steps) val (Node y l r) = Node y (set steps val l) r
set (StepR:steps) val (Node y l r) = Node y l (set steps val r)
...
search' val Empty _ = Nothing
search' val (Node y l r) steps 
  | val == y = Just steps
search' val (Node y l r) steps 
  | left /= Nothing = left
  where left = search' val l (steps ++ [StepL])
search' val (Node y l r) steps 
  | right /= Nothing = right
  | otherwise = Nothing
  where right = search' val r (steps ++ [StepR])
